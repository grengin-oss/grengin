...
<script lang="ts">
  ...
  /* Authentication (SSO) lives inside CONFIGURE, last in that group — it
     configures how people get in, next to who they are and what they may do. */
  const authenticationMenuItem: AdminMenuItem = {
    id: "settings",
    path: "/admin/settings",
    label: $_("sidebar.authentication"),
  ...
  let configureItems = $derived<AdminMenuItem[]>(
    [
      ...
      canViewSettings ? authenticationMenuItem : null,
    ].filter(isMenuItem),
  );
  ...
  let adminMenuItems = $derived<AdminMenuItem[]>([
    ...
    // CONFIGURE section — Authentication is the last entry inside it.
