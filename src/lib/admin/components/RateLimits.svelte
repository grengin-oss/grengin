  ...
  async function handleSubmit() {
    try {
      const data = {
        ...
        scope_id:
          formData.scope !== "global" && formData.scope_id
            ? formData.scope_id
            : undefined,
        requests_per_minute: formData.requests_per_minute,
        requests_per_hour: formData.requests_per_hour
          ? parseInt(formData.requests_per_hour.toString())
          : undefined,
        requests_per_day: formData.requests_per_day
          ? parseInt(formData.requests_per_day.toString())
          : undefined,
        tokens_per_day: formData.tokens_per_day
          ? parseInt(formData.tokens_per_day.toString())
          : undefined,
...
<div class="rate-limits-container">
  ...
  {:else if error}
    <AdminPanelCard>
      <ErrorMessage message={error} onretry={loadRateLimits} />
    </AdminPanelCard>
  {:else if filteredRateLimits.length === 0}
    <AdminPanelCard>
      <AdminEmptyState
        title={filterScope
          ? `No rate limits for ${getScopeLabel(filterScope)}`
    ...
    >
      ...
      <div class="form-group">
        ...
        <input
          ...
          placeholder={formData.scope === "global"
            ? "-"
            : formData.scope === "user"
      ...
      <div class="form-group">
        ...
        />
        <p class="form-help">
          Defaults to [60x/1440x] minute rate if left blank.
        </p>
      </div>
