<script lang="ts">
  ...
  async function loadProviders() {
    try {
      ...
      toast.error(err.message || "Failed to load SSO providers");
    } finally {
      loading = false;
    }
  }
  ...
  async function handleSubmit() {
    try {
      ...
      toast.error(err.message || "Failed to save SSO provider");
    }
  }
  ...
  async function toggleProviderStatus(provider: SsoProvider) {
    try {
      await updateSsoProvider(provider.id, { is_enabled: !provider.is_enabled });
          ...
          {#each providers as provider (provider.id)}
            <tr>
              ...
              <td>
                ...
                >
                  <span class="toggle-label">{provider.is_enabled ? "ON" : "OFF"}</span>
