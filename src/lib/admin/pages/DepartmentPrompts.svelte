<script lang="ts">
  ...
  import { getAdministeredDepartments } from "../../api/admin/departments.js";
  ...
  async function loadInitialData() {
    loading = true;
    try {
      const [deptRes, promptsRes] = await Promise.all([
        getAdministeredDepartments(),
        getRolePrompts(),
      ]);
  ...
  async function loadAssignedPrompts() {
    ...
    try {
      assignedPrompts = await getDepartmentPrompts(selectedDepartmentId);
    } catch (err) {
      const msg =
        err instanceof ApiError
          ? getLocalizedError(err, "description", () => "")
  ...
  async function handleAssignPrompt(promptId: string) {
    ...
    try {
      await assignDepartmentPrompt({
        ...
        prompt_id: promptId,
        priority: maxPriority + 1073741824,
      });
      toast.success($_('admin.departmentPrompts.toast.assigned'));
      await loadAssignedPrompts();
    } catch (err) {
      const msg =
        err instanceof ApiError
          ? getLocalizedError(err, "description", () => "")
  ...
  async function handleDrop(event: DragEvent, targetIndex: number) {
    ...
    for (let i = 0; i < items.length; i++) {
      const newPriority = (i + 1) * 1073741824;
...
<div class="dept-prompts-container">
  <PageHeader
    title={$_('admin.departmentPrompts.title')}
    subtitle={$_('admin.departmentPrompts.subtitle')}
  />
  ...
  {:else}
    ...
    <div class="department-selector">
      <label for="dept-select" class="selector-label">
        {$_('admin.departmentPrompts.selectDepartment')}
      </label>
      <select
        id="dept-select"
    ...
    {#if selectedDepartmentId}
      <div class="content-grid">
        ...
        <div class="panel assigned-panel">
          ...
          {:else if sortedAssigned.length === 0}
            <AdminEmptyState
        ...
        {#if sortedAssigned.length > 0}
          <div class="panel preview-panel">
            <div class="panel-header">
              <h3>{$_('admin.departmentPrompts.previewTitle')}</h3>
            </div>
            <div class="preview-description">
              <p>{$_('admin.departmentPrompts.previewDescription')}</p>
            </div>
                ...
                {#if rp}
                  <div class="preview-card">
                    <div class="preview-card-header">
                      <span class="preview-priority">#{index + 1}</span>
                      <span class="preview-card-name">{rp.name}</span>
                      {#if rp.is_system}
                        <span class="type-pill system">{$_('admin.departmentPrompts.systemType')}</span>
                      {:else}
                        <span class="type-pill user">{$_('admin.departmentPrompts.userType')}</span>
                      {/if}
                    </div>
                    <div class="preview-card-text">
                      {rp.prompt_text}
                    </div>
                    {#if rp.variables.length > 0}
                      <div class="preview-card-vars">
                        {#each rp.variables as v}
    ...
    {:else}
      <AdminEmptyState
        title={$_('admin.departmentPrompts.selectDepartmentTitle')}
        message={$_('admin.departmentPrompts.selectDepartmentMessage')}
      >
        {#snippet icon()}
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
