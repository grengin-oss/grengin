...
sync_android_version() {
  ...
  app_version="$(sed -n 's/^[[:space:]]*"version": "\([^"]*\)",/\1/p' "$ROOT_DIR/src-tauri/tauri.conf.json" | head -n 1)"
  if [[ ! "$app_version" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
    fail "invalid Tauri app version: $app_version"
  fi
  ...
  printf '%s\n' \
    ...
    > "$ROOT_DIR/src-tauri/gen/android/app/tauri.properties"
}
